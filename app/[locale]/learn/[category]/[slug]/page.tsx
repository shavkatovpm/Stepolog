...
import { getArticleBySlug, getAllLearnSlugsWithCategory, type FAQ } from "@/lib/content";
...
export async function generateStaticParams() {
  ...
  return locales.flatMap((locale) =>
    getAllLearnSlugsWithCategory(locale).map((item) => ({
      locale,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${prefix}/learn/${category}/${slug}` },
    openGraph: {
      title: article.meta.title,
      description: article.meta.description,
...
export default async function LearnArticlePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    publisher: { "@type": "Organization", name: "Stepolog", url: "https://stepolog.uz" },
    mainEntityOfPage: `https://stepolog.uz${prefix}/learn/${category}/${slug}`,
  };
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: locale === "uz" ? "Asosiy sahifa" : "Главная", item: `https://stepolog.uz${prefix}` },
      { "@type": "ListItem", position: 2, name: locale === "uz" ? "Startap asoslari" : "Основы стартапа", item: `https://stepolog.uz${prefix}/learn` },
      { "@type": "ListItem", position: 3, name: cat?.title || category, item: `https://stepolog.uz${prefix}/learn/${category}` },
      { "@type": "ListItem", position: 4, name: article.meta.title, item: `https://stepolog.uz${prefix}/learn/${category}/${slug}` },
    ],
  };
